...
dados_dsa = [('Carlos', 'Estrela', 'M', 30), ('Tatiana',
                                              'Moraes', 'F', 41), ('Renato', 'Carvalho', 'M', 62)]
...
dsa_rdd = df.rdd.map(lambda x: (
    x["primeiro_nome"] + " " + x["sobrenome"], x["genero"], x["salario"] * 2))
...
dsa_rdd = df.rdd.map(lambda x: (x.primeiro_nome + " " +
                     x.sobrenome, x.genero, x.salario * 2))
