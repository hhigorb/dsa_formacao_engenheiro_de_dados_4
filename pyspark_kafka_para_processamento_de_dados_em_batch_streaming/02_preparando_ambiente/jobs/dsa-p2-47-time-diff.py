...
df2.withColumn('DiffInMinutes', round(
    col('DiffInSeconds') / 60)).show(truncate=False)

# Calculando a diferença em horas usando a diferença de segundos calculada anteriormente
df2.withColumn('DiffInHours', round(
    col('DiffInSeconds') / 3600)).show(truncate=False)
...
df3 = spark.createDataFrame(data=dsa_horas, schema=[
