...
df1.write.option("header", True).mode(
    "overwrite").csv("/opt/spark/data/zipcodes-estado")

df2 = df.repartition(3, "estado")

print("Número de Partições do df2: " + str(df2.rdd.getNumPartitions()))

df2.write.option("header", True).mode("overwrite").csv(
    "/opt/spark/data/zipcodes-estado-3estados")

df3 = df.repartition("estado")

print("Número de Partições do df3: " + str(df3.rdd.getNumPartitions()))

df3.write.option("header", True).mode("overwrite").csv(
    "/opt/spark/data/zipcodes-estado-todosestados")
