...
df_dsaPartition = spark.read.option(
    "header", True).csv("/opt/spark/data/partition1")
...
df_dsaSinglePart = spark.read.option("header", True).csv(
    "/opt/spark/data/partition2/estado=SC/cidade=BLUMENAU")
...
parqdf_dsa = spark.read.option("header", True).csv(
    "/opt/spark/data/partition1")
